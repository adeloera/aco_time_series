...
outreg2 using output/County_Growth_on_Spending_Results.doc, replace ctitle("T-1") dec(5) nonotes title(Table X. Lagged Per Capita Spending on Spending Growth) addnote(NOTE--Table reports regression coefficients with robust standard errors in parentheses. The dependent variable in each regression is the growth in Medicare per capita spending at the county-year level., ***p<0.01 **p<0.05 *p<0.1)

xtreg spend_growth l2.actual_per_capita_costs, r

outreg2 using output/County_Growth_on_Spending_Results.doc, append ctitle("T-2") dec(5)

xtreg spend_growth l3.actual_per_capita_costs, r

outreg2 using output/County_Growth_on_Spending_Results.doc, append ctitle("T-3") dec(5)

xtreg spend_growth l4.actual_per_capita_costs, r

outreg2 using output/County_Growth_on_Spending_Results.doc, append ctitle("T-4") dec(5)

xtreg spend_growth l5.actual_per_capita_costs, r

outreg2 using output/County_Growth_on_Spending_Results.doc, append ctitle("T-5") label dec(5)

**** 

xtreg spend_growth_3yr l3.spend_growth_3yr, r

outreg2 using Output/County_Growth_on_Growth_Results.doc, replace ctitle("") dec(5) nonotes addnote(NOTE--Table reports regression coefficients with robust standard errors in parentheses. The dependent variable in each regression is the 3 year growth rate in Medicare per capita spending at the county-year level., ***p<0.01 **p<0.05 *p<0.1)
